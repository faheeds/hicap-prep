...
| E4-4 | P2 | ✅ | Email reminders | Optional weekly digest to parents ("3 lessons left this week") and streak-risk nudges. | **Done:** `supabase/migrations/20260914000700_email_reminders.sql` adds `email_reminders_opted_in BOOLEAN DEFAULT FALSE` and `retention_email_sent_at TIMESTAMPTZ` to families. `supabase/functions/send-digest/index.ts` (Deno, Resend API) handles two email types: weekly digest for opted-in families (per-student streak summary) and first-time retention warnings for families where `retention_warned_at IS NOT NULL AND retention_email_sent_at IS NULL` (marks sent after delivery to prevent re-sends). `src/store.js` adds `Store.saveEmailPreference(app, optIn)` and loads `email_reminders_opted_in` from families in cloud mode. Parent settings shows an opt-in toggle (cloud mode only) via `emailReminderToggleHTML()` + `toggleEmailReminders()` in `app.html`. `.env.example` documents `RESEND_API_KEY`, `FROM_EMAIL`, and `CRON_SECRET`. `tests/email.test.mjs` covers toggle HTML, Store.saveEmailPreference in local mode, and migration column presence. **Manual setup required** (see `.env.example`): create a Resend account, verify a sender domain, set `RESEND_API_KEY`/`FROM_EMAIL`/`CRON_SECRET` as Supabase project secrets, deploy send-digest, then set up the weekly cron job per the migration comment. **E2-5 unblocked:** once `RESEND_API_KEY` is configured and send-digest is deployed, flip `retention_automation_enabled = true` in `app_settings`. |
| E4-5 | P2 | ✅ | Printable PDF progress report | One-click export of a student's program progress + mastery bars, useful for parents sharing with a tutor. | **Done:** `buildProgressReportHTML(st)` generates a self-contained print-ready HTML document (completion ring SVG, mastery bars per strand, badges earned, last 10 sessions, disclaimer footer). `printProgressReport(studentId)` opens it in a new window and auto-calls `window.print()` after 400ms for layout to settle. "Print" button added to each student row in the parent home's progress table. Works in both cloud and local mode — pure function of the in-memory APP snapshot. `tests/pdf-report.test.mjs` covers: function existence, full HTML document shape, student name in output, disclaimer presence, all three strand labels, session rows, and empty-state handling for a brand-new student. |
...
| ID | Pri | Status | Feature | Description / Acceptance Criteria | Notes for Claude Code |
|---|---|---|---|---|---|
| E5-0 | P1 | ✅ | **Data model refactor: add a level dimension** | `DRILLS` is currently keyed `battery → tier → subtest`. Add `level` as the outermost key: `DRILLS[cogatLevel][battery][tier][subtest]`. Thread a `level` parameter through every function that reads it: `sampleQuestions`, `collectQuestions`, `collectMixedSpeed`, `collectMock`, `weekPlan`. Add a grade/level field to the student roster (parent sets it once when adding a student; map grade → CogAT level via the table above). | **Done:** `DRILLS` now keyed `{13: {verbal: ..., quant: ..., nonverbal: ...}}`. All five functions updated — `sampleQuestions(level,b,t,sub,n)`, `collectQuestions(level,b,t,sub,n)`, `collectMixedSpeed(level,tier)`, `collectMock(level,tier)`, `weekPlan` now embeds `plan.level = student.cogatLevel\|\|13`. `effectiveLevel(level)` helper falls back to 13 for levels without a content bank yet (prevents crashes when a grade-5 student tries to practice before the Level-11 bank ships). `GRADE_TO_LEVEL` and `gradeToLevel()` exported on `window` for test access. Student roster already had `grade` + `cogatLevel` from E1-3; GRADE_OPTIONS stays `[7]` until a second content bank ships (E5-3/E5-4). `tests/level.test.mjs` covers all the above (12 tests). `smoke.test.mjs` updated to use new `DRILLS[level]` structure. 130/130 tests pass. |
...
| ID | Pri | Status | Feature | Description / Acceptance Criteria | Notes |
|---|---|---|---|---|---|
| E6-1 | P1 | ✅ | Founder funnel dashboard | Track signup → free-usage → paid conversion, and program/mock completion rates. This is the data the business plan's financial model depends on. | **Done:** `supabase/functions/admin-metrics/index.ts` — ADMIN_SECRET-protected GET endpoint that queries families/students/attempts in parallel and returns JSON: `families.{total, paid, free, conversionPct, signupsLast7Days, signupsLast30Days}`, `students.total`, `attempts.{total, practice, program, mock, review}`. `src/admin.html` — standalone admin page with a password-type token field, metric cards auto-refreshing every 60 s, and `?token=` bookmark support. `.env.example` documents `ADMIN_SECRET`. `tests/admin.test.mjs` covers auth guard, DB query presence, response shape, and admin.html structure. Manual setup: deploy function, set `ADMIN_SECRET` as a Supabase project secret. |
| E6-2 | P2 | ✅ | Referral program | Give-a-discount/get-a-discount code shared between families. | **Done:** `supabase/migrations/20260914001100_referral_program.sql` — adds `referral_code` (8-char uppercase hex, unique), `referred_by` (nullable referrer code), `referred_count` (integer) to families; backfills existing rows; updates `handle_new_user` trigger to generate a code on signup. `src/store.js` — `loadCloud()` selects the new columns, exposes `referralCode`/`referredBy`/`referredCount` in the APP snapshot, and performs one-time referral capture: if `localStorage["hicap-referral"]` holds a valid `[A-Z0-9]{8}` code that differs from the family's own code, it writes `referred_by` and clears the key. `src/landing.html` and `src/app.html` both capture `?ref=CODE` from the URL into `localStorage["hicap-referral"]` on page load; app.html also strips the param from the address bar. `src/app.html` `parentHomeHTML()` renders a referral section (cloud mode only) with a copy-to-clipboard link and referred-count; shows a "you were referred" hint if `APP.referredBy` is set. `supabase/functions/stripe-webhook/index.ts` `checkout.session.completed` now looks up the purchasing family's `referred_by` and increments the referrer's `referred_count`. Discount delivery is manual (founder creates a Stripe promo code for referred families, who enter it at the already-present promo code field — avoids conflicting with `allow_promotion_codes: true`). `tests/referral.test.mjs` covers migration shape, store field selection and capture logic, landing/app URL capture, UI presence, and webhook increment. Depends on E3-7 |
| E6-3 | P2 | ✅ | Basic analytics | Lightweight page-view and event tracking so the founder funnel dashboard has conversion-path signal beyond what the DB tables already capture. Privacy-first: Plausible Analytics (cookieless, no PII — GDPR/COPPA-safe). Key events: practice started, program day started, mock started, purchase started. | **Done:** `analyticsConsent()`, `_loadPlausible()`, `acceptAnalytics()`, `declineAnalytics()`, `trackEvent(name, props)` added to `app.html`. Analytics loads only when consent is "accepted" and `plausibleDomain` is configured in `window.__HICAP_CONFIG` (via `config.local.js`). `trackEvent()` is a no-op when consent is not accepted or no domain is set. `config.example.js` and `.env.example` document the `PLAUSIBLE_DOMAIN` key. `tests/analytics.test.mjs` covers consent state, no-op pre-consent, event firing post-consent, and script injection guard. E2-6 (consent banner) ships in the same commit — see E2-6. |
| E6-4 | P2 | ⬜ | SEO content pages | "CogAT testing dates by district," "What is HICAP," etc. — organic acquisition content. | Depends on E4-1 existing first |
| E6-5 | P3 | ⬜ | Opt-in badge/streak sharing | Careful, minimal sharing (e.g., a shareable image of a badge) — no student data beyond first name/avatar ever leaves the app. | Needs explicit privacy review before building |
